...
class CMIEstimator(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        # Access validation predictions and labels directly from class attributes
        pred_list, y_list = zip(*outputs)
        #pred_list, y_list = self.val_pred_list, self.val_label_list
        y = torch.cat(y_list)
    ...
    def optimizer_step(self, current_epoch, batch_nb, optimizer, optimizer_idx, second_order_closure=None):
        # 在optimizer.step之前，你可以进行梯度裁剪
        torch.nn.utils.clip_grad_norm_(self.parameters(), 5.0)  # 进行梯度裁剪
        # 判断 pred_cmi 是否包含 NaN
        assert not torch.isnan(pred_cmi).any(), "pred_cmi contains NaN values"
        # 判断 loss_with_next_feature 是否包含 NaN
        assert not torch.isnan(loss_with_next_feature).any(), "loss_with_next_feature contains NaN values"
